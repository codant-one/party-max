...
async function fetchData() {
  ...
  if(route.params.slug && route.path.startsWith('/products/')) {
    ...
    if (categoriesStores.getCategory) {
      ...
      if (isDesktop) {
        bread.value.push({ title: 'Producto', disabled: true, href: '' })
      }
    } else {
      bread.value.push({ title: 'Producto', disabled: true, href: '' })
    }
...
const productsListPath = computed(() => {
        ...
        <VCardTitle class="px-4 px-md-7 py-3 col-recomendaciones">
          <VRow align="center">
            ...
            <VCol cols="4" md="6" class="text-right">
              <NuxtLink :to="productsListPath" class="ms-md-5 tw-no-underline tw-text-tertiary font-size-16 me-3 hover:tw-text-primary">Ver todos</NuxtLink>
            </VCol> 
          </VRow>
        </VCardTitle>
